...
define([
    ...
    models.Nodes = BaseCollection.extend({
        ...
        nodesFiterType:function(nodetype)
        {
          return this.filter(function(node) {return node.get('node_type')==nodetype});
        },
        nodesFiterMac:function(mac)
        {
          return this.filter(function(node) {return node.get('mac') == mac});
        }
    });
    ...
    models.Phymachine = Backbone.Model.extend({
        constructorName: 'Phymachine',
        urlRoot: '/api/phymachines/'
    });

   models.Phymachines = Backbone.Collection.extend({
        constructorName: 'Phymachines',
        model: models.Phymachine,
        url: '/api/phymachines/'
    });

   models.ClusterRoleStatu = Backbone.Model.extend({
        constructorName: 'ClusterRoleStatu',
        urlRoot: '/api/ClusterRoleStatus/'
    });

   models.ClusterRoleStatus = Backbone.Collection.extend({
        constructorName: 'ClusterRoleStatus',
        model: models.ClusterRoleStatu,
        url: '/api/ClusterRoleStatus/'
    });
