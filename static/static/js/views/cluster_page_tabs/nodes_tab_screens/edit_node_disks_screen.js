...
function($, _, i18n, Backbone, utils, models, EditNodeScreen, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate) {
    ...
    var changemyselect=false;  //页面第一次加载需要启用应用按钮
    var selectvalue; 
    var vardiskdirvalue;
    EditNodeDisksScreen = EditNodeScreen.extend({
        ...
        events: {
            ...
            'change #mysqldisk':'changemysqldisk',
            'change #vardisk':'changevardisk'
        },
        changemysqldisk: function()
        {
            changemyselect = true;
            selectvalue = $("#mysqldisk").val();
            this.render();
        },
        changevardisk: function()
        {
            changemyselect = true;
            vardiskdirvalue = $("#vardisk").val();
            this.render();
        },
        hasChanges: function() {
        ...
        savenodeuserset:function(nodeid,userset)
        {
          $.ajax({
                type: "PUT",
                url: "/api/node/updateuserset/",
                data: "id="+nodeid+"&userset="+JSON.stringify(userset),
        ...
        applyChanges: function() {
            ...
            return $.when.apply($, this.nodes.map(function(node) {
                    //保存node的mysql_influxdb_disk
                    var mysqlvalue=$("#mysqldisk").val()=="请选择" ? "" : $("#mysqldisk").val();
                    var vardiskdir=$("#vardisk").val()=="请选择" ? "" : $("#vardisk").val();
                    var userset = {"mysql_disk":mysqlvalue,"var_dir_disk":vardiskdir};
                    this.savenodeuserset(node.get("id"),userset);
                    //更新数据到内存,并不是保存到数据库
                    node.set({user_set:{'mysql_disk':mysqlvalue,"var_dir_disk":vardiskdir}});
                    //Backbone.sync('update',node,{user_set:{'mysql_disk':mysqlvalue,"var_dir_disk":vardiskdir}});
        ...
        initialize: function() {
            ...
            if (this.nodes.length) {
                selectvalue = this.nodes.at(0).get("user_set")?this.nodes.at(0).get("user_set")['mysql_disk']:"请选择";
                vardiskdirvalue = this.nodes.at(0).get("user_set")?this.nodes.at(0).get("user_set")['var_dir_disk']:"请选择";
        ...
        render: function() {
            this.$el.html(this.template({
                ...
                mysqldiskvalue: selectvalue,
                vardiskvalue:vardiskdirvalue
            })).i18n();
