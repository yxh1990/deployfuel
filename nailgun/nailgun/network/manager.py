...
class NetworkManager(object):
    ...
    @classmethod
    def handleipgroup(cls,pxeip,ip):
         #logger.info("pxeip is {0}".format(pxeip))
         #logger.info("ip is {0}".format(ip))
         #logger.info(u"开始替换ipaddrs表中ip字符的最后一位数字")
         newip=""
         lastnum = pxeip.split(".")[3]
         currlist=ip.split(".")
         currlist[3]=lastnum
         newip=(".").join(currlist)
    ...
    def _update_attrs(cls, node_data):
        ...
        for iface in interfaces:
            ...
            if 'interface_properties' in iface:
                current_iface.interface_properties = iface['interface_properties']
