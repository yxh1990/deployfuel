...
class TaskHelper(object):
    ...
    def recalculate_provisioning_task_progress(cls, task):
        #这里的cluster_id 是在objects下的node.py中定义的Node JSON schema
        cluster_nodes = db().query(Node).filter_by(cluster_id=task.cluster_id)
        nodes_progress = [
            n.progress for n in
            cluster_nodes.filter(
    ...
    def nodes_to_deploy(cls, cluster):
        ...
        for node in cluster.nodes:
            if any([node.pending_addition,
                ...
                for role in node.pending_role_list:
                    #controller有update_required
                    #ceph-osd有update_once 
                    #其他类型的cluster的角色是不存在这两个属性
        ...
        #此处的for循环对移动的cluster无效
        #下面的if条件对移动所有的cluster有效
        #默认创建的cluster都是ha模式,但是没有controller角色
        #所以if永远为false.
    ...
    def nodes_in_provisioning(cls, cluster):
        return sorted(filter(
            ...
            cluster.nodes
            #这里面是说过滤cluster下面的nodes，只查询node的status==‘provisioning'
        ), key=lambda n: n.id
           #这个lambda 是表示按照nodes的id排序。默认为升序
        )
    ...
    @classmethod
    def nodes_to_startorstop(cls, cluster):
        nodes_to_deploy = []
        for node in cluster.nodes:
