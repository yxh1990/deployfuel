...
class MyProvisioningSerializer(object):
    ...
    @classmethod
    def get_disk_a_name(cls, volumes):
        diska=volumes[0]
        if diska:
           return diska["name"]

    @classmethod
    def get_disk_other_name(cls,volumes):
        diskothers=volumes[1:len(volumes)-1]
        disk_others_name=[]
        for dk in diskothers:
            if dk:
               disk_others_name.append(dk["name"])

        return ",".join(disk_others_name)
    
    @classmethod
    def re_calculate_volume(cls,volumes):
        #修改sda的free_space和volumes下面的type
        totalspace=0
        volumelist=volumes[0:len(volumes)-1]
        # for volume in volumelist:
        #     totalspace=totalspace+int(volume["free_space"])
        totalspace=int(volumes[0]["free_space"])
        volumes[0]["volumes"][1]["type"]="partition"

        #修改mount到/root的space
        lastvolumes_item=volumes[len(volumes)-1]
        itemvolumes=lastvolumes_item["volumes"]
        swapsize=int(itemvolumes[1]["size"])
        itemvolumes[0]["size"]=totalspace-swapsize
    ...
    def serialize_interfaces(cls, node):
        ...
        for interface in node.nic_interfaces:
            name = interface.name
            # logger.info(interface.ip_addr)
            # logger.info(interface.mac)
            interfaces[name] = {
            ...
            if interface.mac == node.mac:
                ...
                if interface.netmask:
                   interfaces[name]['netmask']=interface.netmask
                if interface.ip_addr:
                   interfaces[name]['ip_address']=interface.ip_addr
