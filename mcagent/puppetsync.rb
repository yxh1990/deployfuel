  ...
  module Agent
    class Puppetsync < RPC::Agent
      ...
      action 'rsync' do
        ...
        ['manifests','modules'].each do |type|
          ...
          #cmd = "rsync -a  -e 'ssh -p22' root@10.20.0.2:/etc/puppet/2014.2-6.0/#{type} /etc/puppet/#{type}"
          #cmd = "scp -r root@10.20.0.2:/etc/puppet/2014.2-6.0/#{type}/ /etc/puppet/#{type}/"
          run_and_respond(cmd)
        end
