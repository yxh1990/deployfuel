    ...
    ScreenMixin = {
        ...
        returnToNodeList: function() {
            if (this.hasChanges()) {
                ...
                //dialogs.DiscardSettingsChangesDialog({cb: _.bind(this.goToNodeList, this)});
            } else {
                this.goToNodeList();
            }
        }
    ...
    EditNodeInterfacesScreen = React.createClass({
        ...
        statics: {
            fetchData: function(options) {
                var cluster = options.model;
                var nodeIds = utils.deserializeTabOptions(options.screenOptions[0]).nodes.split(',').map(function(id) {return parseInt(id, 10);});
                var nodeLoadingErrorNS = 'cluster_page.nodes_tab.configure_interfaces.node_loading_error.';
                var nodes;
                            ...
                            <div className='network-box-name'>
                                {this.props.bondingAvailable ?
                                    <controls.Input
                                        type='checkbox'
                                        label={ifc.get('name')}
                                        labelClassName='pull-left changebondcolor'
